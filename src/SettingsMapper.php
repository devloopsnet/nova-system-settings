...
class SettingsMapper extends SpatieSettingsMapper
{
    ...
    public function load(string $settingsClass): Collection
    {
        ...
        $properties = $this->fetchProperties($settingsClass, $config->getReflectedProperties()
                                                                    ->keys());
    ...
    public function save(
        ...
        $notRejectedProperties = $properties->reject(fn($payload, string $name) => $config->isLocked($name));

        $changedProperties = $notRejectedProperties->map(function ($payload, string $name) use ($config) {
        ...
        $config->getRepository()
               ->updatePropertiesPayload($config->getGroup(), $changedProperties);

        return $this->fetchProperties($settingsClass, $config->getLocked())
    ...
    public function fetchProperties(string $settingsClass, Collection $names): Collection
    {
        ...
        return collect($config->getRepository()
    ...
    private function ensureNoMissingSettings(
        ...
        $missingSettings = $config->getReflectedProperties()
                                  ->keys()
